        ...
        for line in lines:
            ...
            if len(parts) >= 6 and all(part.isdigit() or (part[0] == '-' and part[1:].isdigit()) for part in [parts[0], parts[2], parts[3], parts[4], parts[5]]):
                try:
                    ...
                    # This catch is technically redundant due to the all() check, but kept for robustness
...
try:
    ...
    if not df.empty:
        df['Unwanted'] = structure_line if structure_line is not None else "" # Assign empty string if no structure line found
    else:
        print(f"Warning: DataFrame for {ct_filepath} is empty.")


    # Correctly assign values to the 'Fold' column
    # Ensure 'Unwanted' column exists and has enough characters
    if 'Unwanted' in df.columns and len(df['Unwanted'].iloc[0]) >= len(df):
      for i in range(len(df)):
        df.loc[i, 'Fold'] = df['Unwanted'].iloc[0][i]
    else:
        # Handle the case where 'Unwanted' column is missing or too short
        df['Fold'] = '' # Assign empty string or handle as appropriate
        df['Fold_Letter'] = 'X'
